...
class QuotaManager private constructor(private val context: Context) {
    ...
    /** Enregistre du temps passé dans une section Reels. */
    fun recordReelTime(durationMs: Long) {
        if (durationMs <= 0L) return
        prefs.edit {
            putLong(KEY_TIME_TODAY_MS, prefs.getLong(KEY_TIME_TODAY_MS, 0L) + durationMs)
        }
                ...
                scope.launch {
                    ...
                    if (existing != null) {
                        db.dailyStatsDao().upsert(
                            existing.copy(quotaMetTime = metTime, streakDay = getStreakDays())
                        )
                    }
                }
            }
    ...
    private fun updateStreak(lastResetDate: String) {
        try {
            ...
            if (lastResetDate == yesterday && quotaMetYesterday) {
                prefs.edit { putInt(KEY_STREAK_DAYS, prefs.getInt(KEY_STREAK_DAYS, 0) + 1) }
    ...
    fun isScheduleBlocked(): Boolean {
        ...
        return getScheduleRules().any { it.isBlocked(LocalTime.now()) }
    }
    ...
    private fun getScheduleBlockMessage(): String {
        val activeRule = getScheduleRules().firstOrNull { it.isBlocked(LocalTime.now()) }
        return if (activeRule != null) "Bloqué de ${activeRule.startHour}h à ${activeRule.endHour}h" else ""
    }
